...
from network import LightningVAutoencoderCombineBENetwork
...
for idx, current_data in enumerate(all_data):
    ...
    wandb.init(name=f'wasif_data_separate_endend_bl', project='wasif_data', group=f'vae separate endend_bl'
    ...
    for unique_label in unique_labels:
        wandb.init(name=f'wasif_data_separate_{unique_label} endend_bl', project='wasif_data',
                   group=f'vae separate endend'
                         f'{current_dataset} {type_data} '
                         f'{"impute" if is_impute else ""}'
                         f'num layers: {num_layers} '
                         f'latent:{autoencoder_latent_shape} '
        ...
        network = LightningVAutoencoderCombineBENetwork(features.shape[1], encoded_labels.shape[1],
