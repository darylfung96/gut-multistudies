...
# pre-trained with secondary losses
wandb.init(name=f'wasif_data_multi_task', project='wasif_data', group=f'multi task nn separate study '
...
network = LightningMultiTaskNetwork(scaled_features.shape[1], encoded_labels.shape[1], one_hot_study_index.shape[1],
                                        num_layers, hidden_size)
network.set_train_secondary_only(True)
tensor_current_data = torch.from_numpy(current_data)
tensor_one_hot_study_index = torch.from_numpy(one_hot_study_index)
tensor_encoded_labels = torch.from_numpy(encoded_labels)
all_dataset = TensorDataset(tensor_current_data, tensor_encoded_labels, tensor_one_hot_study_index)
...
trainer.fit(network, all_dataloader)
best_weights = deepcopy(network.best_weights)

for unique_label in unique_labels:
    wandb.init(name=f'wasif_data_multi_task_{unique_label}', project='wasif_data', group=f'multi task nn separate study '
                                                                          f'{current_dataset} {type_data} '
                                                                          f'{"impute" if is_impute else ""} '
                                                                          f'num layers: {num_layers}'
                                                                          f'latent:{autoencoder_latent_shape} '
    ...
    val_index = data.index[data['Study_name'] == unique_label].values
    ...
    network = LightningMultiTaskNetwork(scaled_features.shape[1], encoded_labels.shape[1], one_hot_study_index.shape[1],
                                        num_layers, hidden_size)
    network.set_train_secondary_only(False)
    network.set_best_weights(best_weights)
...
#                                                                          f'hidden: {hidden_size}', reinit=True)
# plot roc curve
    encoded_y_true_list = label_encoder.transform(labels[val_index]).toarray()
    fpr, tpr, _ = metrics.roc_curve(encoded_y_true_list.ravel(), predicted.ravel())
    ...
    for i in range(encoded_labels.shape[1]):
        y_true = label_encoder.transform(labels[val_index])[:, i]
