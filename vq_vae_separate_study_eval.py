...
for idx, current_data in enumerate(all_data):
    ...
    wandb.init(name=f'wasif_data_conditional_separate', project='wasif_data', group=f'new test scaled vq vae separate '
    ...
    for unique_label in unique_labels:
        wandb.init(name=f'wasif_data_vq_separate_{unique_label}', project='wasif_data',
                   group=f'new test scaled vq vae separate '
                         f'{current_dataset} {type_data} '
                         f'{"impute" if is_impute else ""}'
                         f'num layers: {num_layers} '
                         f'latent:{autoencoder_latent_shape} '
        ...
        with torch.no_grad():
            _, train_tensor_scaled_features, _ = autoencoder_network.get_encoded_features(train_features)
        ...
        with torch.no_grad():
            _, val_tensor_scaled_features, _ = autoencoder_network.get_encoded_features(val_features)
